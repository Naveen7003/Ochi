...
function Navbar() {
  ...
  return (
    <div className={`fixed z-[999] w-full px-20 py-4 font-['NeueMontreal flex justify-between items-center'] transition-all backdrop-opacity-10 backdrop-invert bg-white/30 duration-500 ${navbarVisible ? '' : '-translate-y-full '}`}>
      <div className='logo'>
        {/* Your logo SVG */}
      ...
      <div className='links flex gap-10 ml-20'>
        {["Services", "Our work", "About Us", "Insights", "Contact"].map((item, index) => (
          <a key={index} className={`text-lg capitalize  font-medium ${index === 4 && "mx-32"}`}>{item}</a>
        ))}
      </div>
    </div>
  );
